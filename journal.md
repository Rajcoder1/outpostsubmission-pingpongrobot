...
mm/dd/yy format
Group Project between Raj and Akash
# Day 1 (7/9/26): Starting out (time spent 3 hours)

Brainstormed some ideas for how a ball launcher robot would like and function. We compared different designs on exisitng robots as well and settled on a 2 dof shooter with a turret like design. Additionally, there will be 2 flywheels for front / backspin. 
...
We were also looking for parts and i've roughly decided what I'm planning on using:
...
Today was mostly working on the cad for the ball launcher. We fully developed the launcher module with 2 flywheels along with the pivot structure.
...
Today we finished up the cad for the robot. The design was changed a little bit so now the hopped is directly integrated instead of it being a tube that feeds. Additionally, we made a rotation stand for the base so the servo is more stable along with a change in servo placement for the tilt so that it is more compact.
